...
%   Detailed explanation goes here
    ...
    for i=1 : length(YTest)
        ...
        if(predicted_class == actual_class)
            ...
            fprintf(fileID,[int2str(i), ' out of ', int2str(length(TEST_class_labels)), 'done, correctly classify \n']);
        else
            disp([int2str(i), ' out of ', int2str(length(TEST_class_labels)), 'done, misclassified ']);
            fprintf(fileID,[int2str(i), ' out of ', int2str(length(TEST_class_labels)), 'done, misclassified \n']);
    ...
    fileID = fopen('result.txt','a');
    fprintf(fileID,['The Error Rate was ' num2str(error_rate) '.']);
    fclose(fileID);
